...
class Webservice_model extends CI_Model 
{
    ...
    function get_country_code_by_phone_no($mobile) {
        $this->db->select('*');
        $this->db->from('registration');
        $this->db->where('mobile',$mobile);
        return $this->db->get()->row()->country_code;
    }
    ...
    {  
        if(isset($request['my_team']) && $request['my_team'] == "1")
        {
            if($request['designationid'] == "0")
            { 
    ...
    function get_myteam_player_id($request)
    {
        $this->db->select('player_id');
        $this->db->where('user_id',$request['user_id']);
        if(isset($request['my_team_id'])) {
            $this->db->where('my_team_id',$request['my_team_id']);
        }
...
WHERE contestid = '".$request['contest_id']."' 
    ...
    function update_app(){
        ...
        $this->db->select('id,note,old_version,new_version,apk_name,apk_url,maintenance_status');
